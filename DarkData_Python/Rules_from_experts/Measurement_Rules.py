...
with pd.ExcelFile('X Applicability to Phenomena from experts.xlsx') as xls:
    ...
    Rules['Measurement'] = pd.read_excel(xls, 'Measurement_shen', skiprows=[8,9,10,11],index_col=0,na_values=['NA'])
...
for i in range(0,len(events)):
    ...
    for j in range(0,len(measurements)):
        ...
        "(?variable dd:measurement ddmeasurement:"+str(Rules['Measurement'][measurements[j]].name).replace(' ','+')+"),"+"\n"+
